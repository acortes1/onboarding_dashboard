...
def load_custom_css():
    ...
    if THEME == "light":
        ...
        LOGOUT_BTN_BG = "#F2DEDE"; LOGOUT_BTN_TEXT = "#A94442"; LOGOUT_BTN_BORDER = "#A94442"; LOGOUT_BTN_HOVER_BG = "#EBCFCF"
        ...
        LOGOUT_BTN_BG = "#5A2222"; LOGOUT_BTN_TEXT = "#FFBDBD"; LOGOUT_BTN_BORDER = "#FFBDBD"; LOGOUT_BTN_HOVER_BG = "#6B3333"
    ...
    <style>
        :root {{
            /* CSS variables make it easy to reuse colors */
            ...
            --table-cell-padding: {TABLE_CELL_PADDING}; --table-font-size: {TABLE_FONT_SIZE};
            ...
            --google-btn-bg: {GOOGLE_BTN_BG}; --google-btn-hover-bg: {GOOGLE_BTN_HOVER_BG};
            --google-btn-shadow: {GOOGLE_BTN_SHADOW};
        }}
        ...
        h1 {{
            text-align: center; padding: 0.8em 0.5em; font-size: 2.3rem; letter-spacing: 0.5px;
            border-bottom: 2px solid var(--primary-color); margin-bottom: 1.5em; font-weight: 700;
        }}
        h2 {{
            font-size: 1.7rem; margin-top: 2.2em; margin-bottom: 1.3em; padding-bottom: 0.5em;
        ...
        div[data-testid="stMetric"], .metric-card {{
            background-color: var(--secondary-background-color); padding: 1.4em 1.7em; border-radius: 10px;
            border: 1px solid var(--border-color); box-shadow: 0 5px 12px rgba(0,0,0,0.06);
        ...
        div[data-testid="stButton"] > button, div[data-testid="stDownloadButton"] > button {{
            border: none; padding: 10px 22px; border-radius: 8px; font-weight: 600; transition: all 0.2s ease;
        ...
        .transcript-pane-container {{
            overflow-x: auto; border: 1px solid var(--table-border-color); border-radius: 10px; max-height: 400px;
            padding: 1.5em; background-color: var(--secondary-background-color);
        }}
        .transcript-container {{ max-height: 350px; overflow-y: auto; }}

        /* Style our custom HTML table */
        .custom-table-container {{
            overflow-x: auto; border: 1px solid var(--table-border-color); border-radius: 10px; max-height: 500px;
        }}
        ...
        .custom-styled-table th, .custom-styled-table td {{
            padding: var(--table-cell-padding); text-align: left; border-bottom: 1px solid var(--table-border-color);
        ...
        .login-box {{
            background-color: var(--login-box-bg); padding: 2.5em 3em; border-radius: 15px;
            box-shadow: var(--login-box-shadow); max-width: 450px; width: 100%;
        }}
...
def check_login_and_domain():
    ...
    if not user_email:
        # Handle cases where email couldn't be retrieved.
...
def authenticate_gspread_cached():
    ...
    try:
        ...
        if missing_keys:
            st.error(f"🚨 Error: GCP secrets dict missing keys: {', '.join(missing_keys)}.")
            return None
        ...
        st.error(f"🚨 Error Processing GCP Secrets or Authenticating: {e}. Check format/permissions.")
        return None
            ...
            for fmt in common_formats:
                try:
                    ...
                    if dates.notnull().all():
                        break  # Stop if all are converted.
                except ValueError:
                    continue  # Ignore formats that don't match.
            if dates.notnull().all():
                break
    return dates
...
def format_datetime_to_pst_str(dt_series):
    ...
    def convert_element(element):
        if pd.isna(element):
            return None  # Pass through nulls.
        try:
            ...
            return str(element)  # Fallback to string on error.
...
def load_data_from_google_sheet():
    ...
    if gc is None:
        return pd.DataFrame(), None  # Bail out if auth failed.
    ...
    try:
        ...
        for req_key_internal in KEY_REQUIREMENT_DETAILS.keys():
            column_name_map_to_code[req_key_internal.lower()] = req_key_internal
        ...
        for name_col in ['repName', 'contactName']:
            if name_col in df.columns:
                df[name_col] = df[name_col].apply(capitalize_name)

        # 7. Clean up string columns, ensuring they are strings and replacing NAs.
        string_cols = ['status', 'clientSentiment', 'repName', 'storeName', 'licenseNumber', 'fullTranscript', 'summary',
...
def calculate_metrics(df_input):
    ...
    if df_input.empty:
        return 0, 0.0, pd.NA, pd.NA  # Return defaults for empty input.
    total = len(df_input)
...
def get_default_date_range(date_series):
    ...
    min_date, max_date = (pd.to_datetime(date_series, errors='coerce').dt.date.dropna().min(),
                          pd.to_datetime(date_series, errors='coerce').dt.date.dropna().max()) if date_series is not None and date_series.notna().any() else (None, None)
...
for f_key in categorical_filter_keys:
    st.session_state.setdefault(f_key, [])  # Initialize filters as empty lists.
search_field_keys = ['licenseNumber_search', 'storeName_search']
for s_key in search_field_keys:
    st.session_state.setdefault(s_key, "")  # Initialize search as empty strings.
...
if not st.session_state.data_loaded:
    ...
    if load_time:  # Check if loading was attempted.
        ...
        if not df_loaded.empty:  # If data was loaded successfully.
            ...
            min_d, max_d = (pd.to_datetime(df_loaded['onboarding_date_only'], errors='coerce').dt.date.dropna().min(),
                            pd.to_datetime(df_loaded['onboarding_date_only'], errors='coerce').dt.date.dropna().max()) if 'onboarding_date_only' in df_loaded and df_loaded['onboarding_date_only'].notna().any() else (None, None)
...
ln_search_val = st.sidebar.text_input(
    ...
    key="licenseNumber_global_search_widget_v4_3_1",
...
selected_store_val = st.sidebar.selectbox(
    ...
    key="storeName_global_search_widget_select_v4_3_1",
...
selected_date_range_tuple = st.sidebar.date_input(
    ...
    key="date_selector_custom_v4_3_1", disabled=global_search_active,
...
for col_key, label_text in category_filters_map.items():
    ...
    if not df_original.empty and col_key in df_original.columns and df_original[col_key].notna().any():
        if col_key == 'status':
            options_for_multiselect = sorted([val for val in df_original[col_key].astype(str).str.replace(r"✅|⏳|❌", "", regex=True).str.strip().dropna().unique() if str(val).strip()])
        else:
    ...
    new_selection_multiselect = st.sidebar.multiselect(
        ...
        options=options_for_multiselect, default=valid_current_selection,
        key=f"{col_key}_category_filter_widget_v4_3_1",
        disabled=global_search_active or not options_for_multiselect,
        help=f"Select {label_text}." if options_for_multiselect else f"No {label_text} data."
    )
    ...
    if not global_search_active and new_selection_multiselect != valid_current_selection:
        ...
        st.rerun()
    elif global_search_active and st.session_state.get(f"{col_key}_filter") != new_selection_multiselect:
        st.session_state[f"{col_key}_filter"] = new_selection_multiselect
...
if st.sidebar.button("🧹 Clear Filters", on_click=clear_all_filters_and_search_v4_3_1, use_container_width=True, key="clear_filters_button_v4_3_1", type="primary"):
    st.rerun()
...
if st.sidebar.button("Refresh Data from Source", key="refresh_data_button_v4_3_1", use_container_width=True, type="primary"):
    ...
    st.session_state.df_original = pd.DataFrame()
    clear_all_filters_and_search_v4_3_1()
    st.rerun()  # Clear cache, state, and rerun.
...
if hasattr(st.user, "email") and st.user.email:
    ...
    if hasattr(st.user, "name") and st.user.name and st.user.name.strip():
        try:
            first_name = st.user.name.split()[0]; user_display_name = first_name
        except IndexError:
            user_display_name = st.user.name
...
if global_search_active:
    search_terms = []
    if st.session_state.get("licenseNumber_search", ""):
        search_terms.append(f"License: '{st.session_state['licenseNumber_search']}'")
    if st.session_state.get("storeName_search", ""):
        search_terms.append(f"Store: '{st.session_state['storeName_search']}'")
    ...
    if min_data_dt_summary and max_data_dt_summary and start_dt_filter == min_data_dt_summary and end_dt_filter == max_data_dt_summary and st.session_state.get('date_filter_is_active', False):
    ...
    if not any(st.session_state.get(f"{key}_filter") for key in category_filters_map) and not (st.session_state.get('date_filter_is_active', False) or (start_dt_filter != default_s_init or end_dt_filter != default_e_init)):
        summary_parts.append("No category filters.")
final_summary_message = " | ".join(filter(None, summary_parts))
if not final_summary_message:
    final_summary_message = "Displaying data (default date range)."
...
if not df_original.empty:
    if global_search_active:  # Apply global search if active.
    ...
    else:  # Apply sidebar filters if global search is not active.
        ...
        if 'onboarding_date_only' in df_temp_filters.columns and df_temp_filters['onboarding_date_only'].notna().any():
            ...
            if valid_dates_mask.any():
                date_filter_condition[valid_dates_mask] = (date_objects_for_filter[valid_dates_mask] >= start_dt_filter) & (date_objects_for_filter[valid_dates_mask] <= end_dt_filter)
            df_temp_filters = df_temp_filters[date_filter_condition]
        # Apply category filters.
        for col_name_cat, _ in category_filters_map.items():  # *** FIX: Use col_name_cat here instead of col_key ***
...
def display_html_table_and_details(df_to_display, context_key_prefix=""):
    ...
    if df_to_display is None or df_to_display.empty:
        ...
        if not df_original.empty:
            st.markdown(f"<div class='no-data-message'>📊 No data for {context_name_display}. Try different filters! 📊</div>", unsafe_allow_html=True)
        return
    ...
    if 'status' in df_display_copy.columns:
        ...
        df_display_copy['status_styled'] = ""

    # Define the order of columns in the table and select those present.
    preferred_cols_order = ['onboardingDate', 'repName', 'storeName', 'licenseNumber', 'status_styled', 'score', 'clientSentiment',
    ...
    final_display_cols = list(dict.fromkeys(final_display_cols))

    if not final_display_cols or df_display_copy[final_display_cols].empty:
        context_name_display = context_key_prefix.replace('_', ' ').title().replace('Tab','').replace('Dialog','')
        st.markdown(f"<div class='no-data-message'>📋 No columns/data for {context_name_display}. 📋</div>", unsafe_allow_html=True)
        return
    ...
    for req_key, details in KEY_REQUIREMENT_DETAILS.items():
        column_display_names[req_key] = details.get("chart_label", req_key)
    ...
    for _, row in df_display_copy.iterrows():
        ...
        for col_id in final_display_cols:
            ...
            if col_id == 'score' and pd.notna(cell_value):
                cell_value = f"{float(cell_value):.1f}"  # Format numbers.
            elif col_id == 'days_to_confirmation' and pd.notna(cell_value):
                cell_value = f"{float(cell_value):.0f}"
    ...
    if auto_selected_this_run and not st.session_state.get(auto_selected_once_key, False):
        st.session_state[auto_selected_once_key] = True; st.rerun()  # Rerun once on auto-select.
    elif len(df_display_copy) != 1:
    ...
    if 'fullTranscript' in df_display_copy.columns or 'summary' in df_display_copy.columns:
        ...
        if transcript_options_map:
            ...
            try:
                ...
                current_index_for_select = 0; st.session_state[transcript_session_key_local] = None
            selected_key_from_display = st.selectbox(
                "Select record to view details:",
                options=options_list_for_select, index=current_index_for_select,
                format_func=lambda x: "📄 Choose an entry..." if x is None else x,
                key=f"transcript_selector_{context_key_prefix}_widget_v4_3_1"
            )
            ...
            if st.session_state[transcript_session_key_local]:
                ...
                summary_items_to_display = {
                    ...
                    "Status": selected_row_details.get('status_styled', "N/A"),
                ...
                summary_html_parts_list.append("</div>")
                ...
                for item_col_name_req in ORDERED_TRANSCRIPT_VIEW_REQUIREMENTS:
                    ...
                    if details_obj:
                        ...
                        val_from_row = selected_row_details.get(item_col_name_req, pd.NA)
                        val_str_lower = str(val_from_row).strip().lower()
                        is_met = val_str_lower in ['true', '1', 'yes', 'x', 'completed', 'done']
                        emoji_char = "✅" if is_met else ("❌" if pd.notna(val_from_row) and val_str_lower != "" else "➖")
                ...
                if transcript_content and transcript_content.lower() not in ['na', 'n/a', '']:
                    html_transcript_parts = ["<div class='transcript-pane-container'><div class='transcript-container'>"]
                    ...
                    html_transcript_parts.append("</div></div>")
    ...
    st.download_button(
        ...
        key=f"download_csv_{context_key_prefix}_button_v4_3_1"
    )
            ...
            with st.container():
                ...
                with chart_cols_1:
                    ...
                    if 'status' in df_filtered.columns and df_filtered['status'].notna().any():
                        status_counts_df = df_filtered['status'].astype(str).str.replace(r"✅|⏳|❌", "", regex=True).str.strip().value_counts().reset_index()
                        status_counts_df.columns = ['status', 'count']
                        status_fig = px.bar(status_counts_df, x='status', y='count', color='status', title="Onboarding Status Distribution", color_discrete_sequence=ACTIVE_PLOTLY_PRIMARY_SEQ)
                    ...
                    if 'repName' in df_filtered.columns and df_filtered['repName'].notna().any():
                        ...
                        rep_counts_df.columns = ['repName', 'count']
                        rep_fig = px.bar(rep_counts_df, x='repName', y='count', color='repName', title="Onboardings by Representative", color_discrete_sequence=ACTIVE_PLOTLY_QUALITATIVE_SEQ)
                ...
                with chart_cols_2:
                    ...
                    if 'clientSentiment' in df_filtered.columns and df_filtered['clientSentiment'].notna().any():
                        ...
                        sent_counts_df.columns = ['clientSentiment', 'count']
                        ...
                        sent_fig = px.pie(sent_counts_df, names='clientSentiment', values='count', hole=0.4, title="Client Sentiment Breakdown", color='clientSentiment', color_discrete_map=current_sentiment_map_plot)
                        ...
                        for item_col_name_chart in actual_key_cols_for_checklist_chart:
                            ...
                            if item_col_name_chart in df_confirmed_for_chart.columns:
                                ...
                                if total_valid_for_item > 0:
                                    checklist_data_for_plotly.append({"Key Requirement": chart_label_bar, "Completion (%)": (true_count_for_item / total_valid_for_item) * 100})
                        if checklist_data_for_plotly:
                            df_checklist_plotly = pd.DataFrame(checklist_data_for_plotly)
                            if not df_checklist_plotly.empty:
                                checklist_bar_fig = px.bar(df_checklist_plotly.sort_values("Completion (%)", ascending=True), x="Completion (%)", y="Key Requirement", orientation='h', title="Key Req Completion (Confirmed Only)", color_discrete_sequence=[PRIMARY_COLOR_FOR_PLOTLY])
                            ...
                            st.markdown("<div class='no-data-message'>📊 No valid checklist items for req chart.</div>", unsafe_allow_html=True)
                    else:
    ...
    if not df_filtered.empty:
        ...
        if 'onboarding_date_only' in df_filtered.columns and df_filtered['onboarding_date_only'].notna().any():
            ...
            if not df_trend_source.empty:
                ...
                if not trend_data_resampled.empty:
                    trend_line_fig = px.line(trend_data_resampled, x='onboarding_datetime', y='count', markers=True, title=f"Onboardings Over Time ({resample_freq} Trend)", color_discrete_sequence=[ACTIVE_PLOTLY_PRIMARY_SEQ[0]])
        ...
        if 'days_to_confirmation' in df_filtered.columns and df_filtered['days_to_confirmation'].notna().any():
            ...
            if not days_data_for_hist.empty:
                ...
                days_dist_fig = px.histogram(days_data_for_hist, nbins=num_bins_hist, title="Distribution of Days to Confirmation", color_discrete_sequence=[ACTIVE_PLOTLY_PRIMARY_SEQ[1]])
