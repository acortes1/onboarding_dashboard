...
# --- Page Configuration ---
st.set_page_config(
    page_title="Onboarding Analytics Dashboard v4.4.2", # Updated Version for export logic fix
    page_icon="📈",
    layout="wide",
...
def load_custom_css():
    ...
    <style>
        ...
        h1 {{
            text-align: center; padding-top: 0.8em; padding-bottom: 0.8em; font-size: 2.4rem;
            letter-spacing: 0.5px; border-bottom: 2px solid var(--primary-color); margin-bottom: 1.5em;
            font-weight: 700;
        }}
        h2 {{
            font-size: 1.8rem; margin-top: 2.2em; margin-bottom: 1.3em;
            padding-bottom: 0.5em; border-bottom: 1px solid var(--border-color); font-weight: 600;
        }}
        h3 {{
            font-size: 1.5rem; margin-top: 2em; margin-bottom: 1.1em; font-weight: 600;
            color: var(--text-color); opacity: 0.9;
        }}
         h5 {{
            color: var(--text-color); opacity: 0.95; margin-top: 1.8em; margin-bottom: 0.9em;
            font-weight: 500; letter-spacing: 0.1px; font-size: 1.1rem;
        }}

        div[data-testid="stMetric"], .metric-card {{
            background-color: var(--secondary-background-color); padding: 1.4em 1.7em;
            border-radius: 10px; border: 1px solid var(--border-color);
            box-shadow: 0 5px 12px rgba(0,0,0,0.06);
            transition: transform 0.2s ease-out, box-shadow 0.2s ease-out; margin-bottom: 1.3em;
        }}
        div[data-testid="stMetric"]:hover, .metric-card:hover {{
            transform: translateY(-5px); box-shadow: 0 8px 16px rgba(0,0,0,0.08);
        }}
        div[data-testid="stMetricLabel"] > div {{
            color: var(--text-color); opacity: 0.85; font-weight: 500; font-size: 0.9rem;
            text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 0.5em;
        }}
        div[data-testid="stMetricValue"] > div {{
            color: var(--text-color); font-size: 2.6rem !important; font-weight: 700; line-height: 1.1;
        }}
        div[data-testid="stMetricDelta"] > div {{
            color: var(--text-color); opacity: 0.8; font-weight: 500; font-size: 0.88rem;
        }}

        div[data-testid="stSidebarUserContent"] {{ padding: 1.8em 1.4em; background-color: var(--secondary-background-color); }}
        div[data-testid="stSidebarUserContent"] h2, div[data-testid="stSidebarUserContent"] h3 {{
             color: var(--primary-color); border-bottom-color: var(--border-color);
        }}
        div[data-testid="stSidebarNavItems"] {{ padding-top: 1.3em; }}

        div[data-testid="stButton"] > button, div[data-testid="stDownloadButton"] > button {{
            border: none; padding: 11px 25px; border-radius: 8px; font-weight: 600;
        ...
        div[data-testid="stButton"] > button:hover {{
            ...
            transform: translateY(-2px); box-shadow: 0 4px 7px rgba(0,0,0,0.1);
        }}
        ...
        div[data-testid="stDownloadButton"] > button:hover {{
            ...
            transform: translateY(-2px); box-shadow: 0 4px 7px rgba(0,0,0,0.09);
        }}
        ...
        .streamlit-expanderHeader {{
            color: var(--text-color) !important; font-weight: 600; font-size: 1.05em; padding: 1em 0.7em;
        }}
        .streamlit-expander {{
            border: 1px solid var(--border-color); background-color: var(--background-color);
            border-radius: 10px; margin-bottom: 1.3em;
        }}
        .streamlit-expander > div > div > p {{ color: var(--text-color); }}

        div[data-testid="stRadio"] label {{
            padding: 11px 20px; margin: 0 4px; border-radius: 8px 8px 0 0;
            border: 1px solid transparent; border-bottom: none;
            background-color: var(--secondary-background-color); color: var(--text-color);
            opacity: 0.8; transition: all 0.25s ease; font-weight: 500; font-size: 1rem;
        }}
        div[data-testid="stRadio"] input:checked + div label {{
            background-color: var(--background-color); color: var(--primary-color);
            font-weight: 600; opacity: 1.0; border-top: 3px solid var(--primary-color);
            border-left: 1px solid var(--border-color); border-right: 1px solid var(--border-color);
            box-shadow: 0 -3px 6px rgba(0,0,0,0.04);
        }}
        div[data-testid="stRadio"] {{
            padding-bottom: 0px; border-bottom: 2px solid var(--primary-color); margin-bottom: 28px;
        }}
        div[data-testid="stRadio"] > label > div:first-child {{ display: none; }}

        .transcript-details-section {{
            margin-left: 20px; padding-left: 20px; border-left: 3px solid var(--primary-color); margin-top: 1.4em;
        }}
        .transcript-summary-grid {{
            display: grid; grid-template-columns: repeat(auto-fit, minmax(270px, 1fr));
            gap: 1.3em; margin-bottom: 2em; color: var(--text-color);
        }}
        .transcript-summary-item {{
            background-color: var(--secondary-background-color); padding: 1.1em 1.3em;
            border-radius: 8px; border: 1px solid var(--border-color);
        }}
        .transcript-summary-item strong {{ color: var(--primary-color); font-weight: 600; }}
        .transcript-summary-item-fullwidth {{
            grid-column: 1 / -1; margin-top: 1.3em; padding-top: 1.3em;
            border-top: 1px dashed var(--border-color);
        }}
        .requirement-item {{
            margin-bottom: 1em; padding: 1em 1.2em; border-left: 4px solid var(--primary-color);
        ...
        .requirement-item .type {{
            font-weight: 500; color: var(--text-color); opacity: 0.75; font-size: 0.8rem;
            margin-left: 12px; background-color: var(--background-color);
            padding: 3px 8px; border-radius: 4px;
        }}
        .transcript-container {{
            background-color: var(--secondary-background-color); color: var(--text-color);
            padding: 2em; border-radius: 10px; border: 1px solid var(--border-color);
            max-height: 580px; overflow-y: auto;
            font-family: 'SFMono-Regular', Consolas, 'Liberation Mono', Menlo, Courier, monospace;
            font-size: 0.9rem; line-height: 1.7; box-shadow: inset 0 2px 6px rgba(0,0,0,0.03);
        }}
        .transcript-line strong {{ color: var(--primary-color); font-weight: 600; }}

        .footer {{
            font-size: 0.9rem; color: var(--text-color); opacity: 0.65; text-align: center;
            padding: 35px 0; border-top: 1px solid var(--border-color); margin-top: 60px;
        }}
        .active-filters-summary {{
            font-size: 0.92rem; color: var(--text-color); opacity: 0.9; margin-top: 0px; margin-bottom: 2.2em;
            padding: 1em 1.4em; background-color: var(--secondary-background-color);
            border-radius: 8px; border: 1px solid var(--border-color); text-align: center;
            box-shadow: 0 3px 7px rgba(0,0,0,0.04);
        }}
        .no-data-message {{
            text-align: center; padding: 35px; font-size: 1.2rem; color: var(--text-color);
            opacity: 0.7; background-color: var(--secondary-background-color);
            border-radius: 8px; border: 1px dashed var(--border-color); margin-top: 1.4em;
        }}
        ...
        div[data-testid="stMultiSelect"] div[role="combobox"] {{
            ...
            padding-top: 0.65em !important; padding-bottom: 0.65em !important;
            font-size: 0.95rem;
        }}
        ...
        div[data-testid="stModal"] > div {{
            border-radius: 12px !important; box-shadow: 0 12px 30px rgba(0,0,0,0.2) !important;
        }}
        div[data-testid="stModalHeader"] {{
            font-size: 1.6rem; color: var(--primary-color); padding-bottom: 0.9em;
            border-bottom: 1px solid var(--border-color); font-weight: 600;
        }}

        .custom-table-container {{
            overflow-x: auto; border: 1px solid var(--table-border-color);
            border-radius: 10px; box-shadow: 0 4px 10px rgba(0,0,0,0.06);
            margin-bottom: 1.8em; max-height: 480px; overflow-y: auto;
        }}
        .custom-styled-table {{
            width: 100%; border-collapse: collapse; font-size: var(--table-font-size);
            color: var(--text-color);
        }}
        .custom-styled-table th, .custom-styled-table td {{
            padding: var(--table-cell-padding); text-align: left;
            border-bottom: 1px solid var(--table-border-color);
            border-right: 1px solid var(--table-border-color); white-space: nowrap;
        }}
        ...
        .custom-styled-table th {{
            ...
            font-weight: 600; text-transform: capitalize; position: sticky; top: 0; z-index: 1;
        }}
        .custom-styled-table tbody tr:hover {{
            background-color: color-mix(in srgb, var(--secondary-background-color) 75%, var(--primary-color) 8%);
        }}
...
def check_password():
    ...
    if app_password is None: st.sidebar.warning("🔑 APP_ACCESS_KEY not set (dev mode)."); return True
    ...
    with form_col:
        ...
        with st.form("password_form_main_app_v4_4_2"): # Key updated
            st.markdown("<h4 style='text-align: center;'>Enter Access Key</h4>", unsafe_allow_html=True)
            password_attempt = st.text_input("Access Key:", type="password", help=app_hint, key="pwd_input_main_app_v4_4_2", placeholder="Enter your key", label_visibility="collapsed") # Key updated
            st.markdown("")
            ...
            if submitted:
                ...
                else: st.error("🚫 Incorrect Access Key. Please try again."); return False
    return False
...
def authenticate_gspread_cached():
    ...
    try:
        ...
        st.error(f"🚨 Error: Could not convert GCP secrets object to dictionary. Type: {type(gcp_secrets_obj)}. Error: {e}")
        return None

    if 'private_key' in gcp_secrets_dict and isinstance(gcp_secrets_dict['private_key'], str):
        gcp_secrets_dict['private_key'] = gcp_secrets_dict['private_key'].replace('\\n', '\n')
    ...
    if missing_keys:
        st.error(f"🚨 Error: GCP secrets dictionary is missing the following required keys: {', '.join(missing_keys)}.")
        return None
    ...
    try:
        ...
        st.error(f"🔑 Google Authentication Error: {e}. This often indicates an issue with the service account key itself (e.g., incorrect, revoked, malformed private key) or with the service account's permissions in GCP (e.g., APIs not enabled, insufficient IAM roles, or problems with the Google Sheet sharing settings). Please verify your GCP configuration and the secrets provided to Streamlit.")
        return None
...
def generate_executive_snapshot_pdf(df_data, mtd_metrics, filtered_metrics, last_refresh_dt, pst_tz):
    ...
    try:
        ...
        if not df_data.empty and 'status' in df_data.columns and df_data['status'].notna().any():
            ...
            status_fig_pdf.update_layout(plotly_base_layout_settings, title_text="Onboarding Status", height=300, width=500, margin=dict(l=40, r=20, t=40, b=40))
            img_bytes = plotly_fig_to_image_bytes(status_fig_pdf)
            if img_bytes:
                pdf.image(io.BytesIO(img_bytes), x=pdf.get_x() + 10, w=chart_img_width * 0.7, type='PNG')
                pdf.ln(2) 
            else:
...
if not st.session_state.data_loaded and st.session_state.last_data_refresh_time is None:
    df_loaded_init = load_data_from_google_sheet() # Renamed to avoid conflict
    if st.session_state.last_data_refresh_time is None: st.session_state.last_data_refresh_time = datetime.now(UTC_TIMEZONE)
    if not df_loaded_init.empty:
        st.session_state.df_original = df_loaded_init; st.session_state.data_loaded = True
        ds, de, min_d, max_d = get_default_date_range(df_loaded_init.get('onboarding_date_only'))
...
df_original = st.session_state.df_original # Use data from session state
...
ln_search_val = st.sidebar.text_input(f"Search {global_search_cols['licenseNumber']}:", value=st.session_state.get("licenseNumber_search", ""), key="licenseNumber_global_search_widget_v4_4_2", help="Enter license number part.") 
...
selected_store_val = st.sidebar.selectbox(f"Search {global_search_cols['storeName']}:", options=store_names_options, index=current_store_idx, key="storeName_global_search_widget_select_v4_4_2", help="Select or type store name.") 
...
if s_col1.button("MTD", key="mtd_button_v4_4_2", use_container_width=True, disabled=global_search_active): 
...
if s_col2.button("YTD", key="ytd_button_v4_4_2", use_container_width=True, disabled=global_search_active): 
...
if s_col3.button("ALL", key="all_button_v4_4_2", use_container_width=True, disabled=global_search_active): 
...
selected_date_range_tuple = st.sidebar.date_input("Custom Date Range (Onboarding):", value=(val_start_widget, val_end_widget), min_value=min_dt_for_widget, max_value=max_dt_for_widget, key="date_selector_custom_v4_4_2", disabled=global_search_active, help="Select start/end dates.") 
...
start_dt_filter, end_dt_filter = st.session_state.date_range # Define after potential update from date_input
...
for col_key, label_text in category_filters_map.items():
    ...
    new_selection_multiselect = st.sidebar.multiselect(f"Filter by {label_text}:", options=options_for_multiselect, default=valid_current_selection, key=f"{col_key}_category_filter_widget_v4_4_2", disabled=global_search_active or not options_for_multiselect, help=f"Select {label_text}." if options_for_multiselect else f"No {label_text} data.") 
...
def clear_all_filters_and_search_v4_4_2(): 
...
if st.sidebar.button("🧹 Clear Filters", on_click=clear_all_filters_and_search_v4_4_2, use_container_width=True, key="clear_filters_button_v4_4_2"): st.rerun() 

# --- Apply Filters and Create df_filtered, df_filtered_for_export ---
# This block is now after all sidebar filter widgets have been processed.
...
if not df_original.empty:
    if global_search_active:
        ...
        if sn_term and "storeName" in df_temp_gs.columns:
        ...
        if 'onboarding_date_only' in df_temp_filters.columns and df_temp_filters['onboarding_date_only'].notna().any():
            ...
            if valid_dates_mask.any():
                date_filter_condition[valid_dates_mask] = (date_objects_for_filter[valid_dates_mask] >= start_dt_filter) & \
...
# Populate df_filtered_for_export based on the final df_filtered
if not df_filtered.empty:
    df_filtered_for_export = df_filtered.copy()
else:
    df_filtered_for_export = pd.DataFrame()

# --- Sidebar: Data Management & Exports (Continued) ---
...
if st.sidebar.button("Refresh Data from Source", key="refresh_data_button_v4_4_2", use_container_width=True): 
    ...
    clear_all_filters_and_search_v4_4_2(); st.rerun()
...
if global_search_active:
    csv_help_text = "CSV export is based on active filters, not global search. Clear global search to export filtered data."
...
if not csv_button_disabled:
    ...
    st.sidebar.download_button(
        ...
        mime="text/csv",
        key="download_filtered_csv_button_sidebar_v4_4_2", 
     ...
     st.sidebar.download_button(
        ...
        mime="text/csv",
        key="download_filtered_csv_button_sidebar_disabled_v4_4_2", 
        use_container_width=True,
        disabled=True,
        help=csv_help_text
    )
...
# PDF Export Button
mtd_metrics_for_pdf = calculate_metrics(df_mtd_data if 'df_mtd_data' in locals() else pd.DataFrame()) # Ensure df_mtd_data is defined
filtered_metrics_for_pdf = calculate_metrics(df_filtered_for_export)

pdf_button_disabled = global_search_active or df_filtered_for_export.empty
pdf_help_text = "Generate and download a PDF summary of key metrics and charts from filtered data."
if global_search_active:
    pdf_help_text = "PDF Snapshot is based on filtered data. Clear global search to enable."
elif df_filtered_for_export.empty:
    pdf_help_text = "No filtered data to generate PDF. Adjust filters."


if st.sidebar.button("📄 Download Executive Snapshot (PDF)", key="generate_pdf_main_button_v4_4_2", use_container_width=True, help=pdf_help_text, disabled=pdf_button_disabled): 
    # If this button is clicked, it means it was not disabled.
    # So, global_search_active is False AND df_filtered_for_export is not empty.
    ...
    if pdf_bytes:
        # Use a unique key for the actual download button to ensure it's fresh
        download_key = f"download_exec_snapshot_pdf_final_button_v4_4_2_{int(time.time())}"
        st.sidebar.download_button(
...
st.sidebar.markdown("---");
st.sidebar.caption(f"Onboarding Dashboard v4.4.2\n\n© {datetime.now().year} Nexus Workflow") 
...
selected_tab = st.radio("Navigation:", ALL_TABS, index=current_tab_idx, horizontal=True, key="main_tab_selector_v4_4_2") 
...
# MTD Calculations (ensure df_original is used for a consistent MTD baseline)
...
def display_html_table_and_details(df_to_display, context_key_prefix=""):
    ...
    if 'fullTranscript' in df_display_copy.columns or 'summary' in df_display_copy.columns:
        ...
        if transcript_options_map:
            ...
            selected_key_from_display = st.selectbox("Select record to view details:", options=options_list_for_select, index=current_index_for_select, format_func=lambda x: "📄 Choose an entry..." if x is None else x, key=f"transcript_selector_{context_key_prefix}_widget_v4_4_2") 
    ...
    if 'status_styled' in df_download_subset.columns and 'status' in df_display_copy.columns:
        df_download_subset['status'] = df_display_copy['status'] 
    ...
    csv_data_to_download_context = convert_df_to_csv(df_download_subset); # Renamed to avoid conflict
    download_label = f"📥 Download These {context_key_prefix.replace('_', ' ').title().replace('Tab','').replace('Dialog','')} Results"
    st.download_button(label=download_label, data=csv_data_to_download_context, file_name=f'{context_key_prefix}_results_{datetime.now().strftime("%Y%m%d_%H%M")}.csv', mime='text/csv', use_container_width=True, key=f"download_csv_{context_key_prefix}_button_v4_4_2") 
...
if st.session_state.get('show_global_search_dialog', False) and global_search_active:
    ...
    def show_global_search_dialog_content():
        ...
        if st.button("Close & Clear Search", key="close_gs_dialog_clear_button_v4_4_2"): 
...
if st.session_state.active_tab == TAB_OVERVIEW:
    ...
    elif not df_filtered.empty: # Use df_filtered here for display
        total_filtered_calc, sr_filtered_calc, score_filtered_calc, days_filtered_calc = calculate_metrics(df_filtered); cols_filtered_overview = st.columns(4) 
    ...
    if global_search_active: st.info("ℹ️ Global Search active. Results in pop-up. Close/clear search for category/date filters here.")
        ...
        display_html_table_and_details(df_filtered, context_key_prefix="filtered_analysis") # Use df_filtered
        st.header("🎨 Key Visualizations (Filtered Data)")
        if not df_filtered.empty: # Use df_filtered
...
st.markdown("---"); st.markdown(f"<div class='footer'>Onboarding Dashboard v4.4.2 © {datetime.now().year} Nexus Workflow. All Rights Reserved.</div>", unsafe_allow_html=True)