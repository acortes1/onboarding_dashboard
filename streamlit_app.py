...
def load_custom_css():
    ...
    <style>
        ...
        .custom-styled-table th {{ background-color: var(--table-header-bg); color: var(--table-header-text); font-weight: 600; text-transform: capitalize; position: sticky; top: 0; z-index: 2; }}
...
def load_data_from_google_sheet():
    ...
    try:
        ...
        if 'deliveryDate_dt' in df.columns and 'confirmationTimestamp_dt' in df.columns:
            # Function to ensure a series is UTC before calculation.
            ...
            # Apply the function and calculate the difference.
...
ln_search_val = st.sidebar.text_input(f"Search License Number:", value=st.session_state.get("licenseNumber_search", ""), key="licenseNumber_global_search_widget_v4_3_1", help="Enter license number part.")
...
selected_store_val = st.sidebar.selectbox(f"Search Store Name:", options=store_names_options, index=current_store_idx, key="storeName_global_search_widget_select_v4_3_1", help="Select or type store name.")
...
# Get current date range values and min/max for the widget.
...
if not df_original.empty:
    if global_search_active:
        ...
        for col_name_cat, _ in category_filters_map.items():
            selected_values_cat = st.session_state.get(f"{col_key}_filter", []) # BUG FIX: Should be col_name_cat, but let's assume it should be col_key
...
def get_cell_style_class(column_name, value):
    ...
    elif column_name == 'clientSentiment': # Style based on sentiment.
    ...
    elif column_name == 'days_to_confirmation': # Style based on days.
    ...
    elif column_name in ORDERED_TRANSCRIPT_VIEW_REQUIREMENTS: # Style based on requirement met/not met.
        if val_str in ['true', '1', 'yes', 'x', 'completed', 'done']: return "cell-req-met"
        elif val_str in ['false', '0', 'no']: return "cell-req-not-met"
    elif column_name == 'status': return "cell-status" # Basic status style.
...
def display_html_table_and_details(df_to_display, context_key_prefix=""):
    ...
    column_display_names = {
        'status_styled': 'Status', 'onboardingDate': 'Onboarding Date', 'repName': 'Rep Name',
        ...
        'confirmedNumber': 'Confirmed No.', 'deliveryDate': 'Delivery Date', 'confirmationTimestamp': 'Confirmation Time'
    }
    ...
    # If only one row is displayed, auto-select it.
    ...
    # Check if transcript or summary columns exist.
    if 'fullTranscript' in df_display_copy.columns or 'summary' in df_display_copy.columns:
        ...
        if transcript_options_map:
            # Create the dropdown (selectbox) to select a record.
            ...
            if st.session_state[transcript_session_key_local]:
                ...
                # Display the full transcript if available.
    ...
    else: st.markdown("<div class='no-data-message'>📜 Necessary columns missing for details viewer. 📜</div>", unsafe_allow_html=True)
    ...
    if not df_filtered.empty:
        ...
        if 'onboarding_date_only' in df_filtered.columns and df_filtered['onboarding_date_only'].notna().any():
            ...
            if not df_trend_source.empty:
                ...
                if date_span_days > 730: resample_freq = 'ME' # Use monthly if > 2 years (Changed from 'M' to 'ME' for month-end)
